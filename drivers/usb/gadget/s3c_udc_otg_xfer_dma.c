 ...
 * Samsung S5P on-chip full/high speed USB OTG 2.0 device controllers
 *
...
static u8 last_ep_in = 0;
static u8 last_ep_out = 0;
...
static inline int setdma_rx(struct s3c_ep *ep, struct s3c_request *req)
{
	u32 *buf, ctrl;
	u32 length, pktcnt;
...
static inline int setdma_tx(struct s3c_ep *ep, struct s3c_request *req)
{
	u32 *buf, ctrl = 0;
	u32 length, pktcnt;
	u32 ep_num = ep_index(ep);
...
static inline void complete_rx(struct s3c_udc *dev, u8 ep_num)
{
	...
	if (is_short || req->req.actual == xfer_length) {
		if ((ep_num != EP0_CON) || (ep_num == EP0_CON && dev->ep0state != DATA_STATE_RECV)) {
			done(ep, req, 0);
			if (!list_empty(&ep->queue)) {
				req = list_entry(ep->queue.next, struct s3c_request, queue);
...
static inline void complete_tx(struct s3c_udc *dev, u8 ep_num)
{
	struct s3c_ep *ep = &dev->ep[ep_num];
...
static inline void process_ep_in_intr(struct s3c_udc *dev)
{
...
static inline void process_ep_out_intr(struct s3c_udc *dev)
{
...
got_ep_out:
	...
	if (ep_num != 0) {
		...
		if (ep_intr_status & TRANSFER_DONE ) {
			...
			/* Set size & address for receiving next control transfer */
			writel((3<<29)|(1 << 19)|sizeof(struct usb_ctrlrequest), S3C_UDC_OTG_DOEPTSIZ(EP0_CON));
			writel(virt_to_phys(&usb_ctrl), S3C_UDC_OTG_DOEPDMA(EP0_CON));

			/*
			 * Set EPENA to enalbe EP0_OUT and Set NAK. Setting SNAK is
			 * a s/w work around becasuse the core of hardware sometimes failed to
			 * set SNAK after receiving setup phase of control transfer.
			 * The failure QUITE OFTEN occured when binded RNDIS function.
			 */
			ep_ctrl = readl(S3C_UDC_OTG_DOEPCTL(EP0_CON));
			writel(ep_ctrl|DEPCTL_EPENA|DEPCTL_SNAK, S3C_UDC_OTG_DOEPCTL(EP0_CON));
		}
		else if (ep_intr_status & CTRL_OUT_EP_SETUP_PHASE_DONE ) {
